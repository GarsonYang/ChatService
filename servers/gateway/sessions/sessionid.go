...
func NewSessionID(signingKey string) (SessionID, error) {
	...
	h.Write(randBytes)
	hashedSessionID := h.Sum(randBytes)

	encodedSessionID := base64.URLEncoding.EncodeToString(hashedSessionID)

	sid := SessionID(encodedSessionID)
...
func ValidateID(id string, signingKey string) (SessionID, error) {
	...
	if len(decodedSessionID) <= idLength {
		return InvalidSessionID, ErrInvalidID
	}

	rawSessionID := decodedSessionID[:idLength]
	previousHashed := decodedSessionID[idLength:]

	h := hmac.New(sha256.New, []byte(signingKey))
	h.Write(rawSessionID)
	rehashed := h.Sum(nil)

	if !hmac.Equal(previousHashed, rehashed) {
		return InvalidSessionID, ErrInvalidID
	}

	NewSessionID := SessionID(id)
